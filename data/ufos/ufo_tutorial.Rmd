...
<h2 class = "intro">Welcome to R!</h2>

We will be working online, because it is a great way to play with R without downloading it. If you want to have R with you at all times, download 
...
Otherwise, navigate to [RStudio Cloud](https://rstudio.cloud/) and click *Get Started*. After you've made your account, you will be navigated to *Your Workspace*. Click the *New Project* button and wait for the *Deploying Project* bubbles to dissappear.

## Purpose of this Tutorial 

R is a great resource that has crazy good documentation including several free books that systematically teach users the basics and then some. This tutorial is not meant to replace any such tutorials but instead to provide an inital project to try out R and R Studio. As such, it will present a streamlined process with jumping off points. If you are interested in learning more R there will be a recommended booklist [at the end](#booklist). 

## Set up the enviroment 

Click on *Global Options* under *Tools*. There will be four sections, *R Sessions*, *Workspace*, *History* and *Other*. Under *Workspace*, uncheck 
 `Restore .RData into workspace at startup` and change `Save workspace to .RData on exit` to to `never`. 

This means that whatever code you create will have to be rerun each time you restart your R session. This will make your code reproducable. In other words, if you run your code from top to bottom, you will get the same result every time. 

## Load the Tidyverse 
<p class = "note"> This tutorial also makes extensive use of the **Tidyverse**. </p>

If you have never used the **Tidyverse** before, you are about to meet one of the best packages in R. Or group of packages. The Tidyverse contains multiple packages designed to clean data, wrangle it and graph it. The packages are compatible with each other, so you can easily move through each stage.

To install the **Tidyverse**, either click on the *Packages* tab in the lower right, then the *install* button and type `tidyverse` or run `install.packages("tidyverse")`. Next, load the package into R using `library()`. This code should go at the top of your R Script. 
```{r library.1}
library(tidyverse)
```
...
We will be working with a semi-cleaned version of a UFO data set from [Kaggle](https://www.kaggle.com/camnugent/ufo-sightings-around-the-world). The original data set can be found [here](https://github.com/planetsig/ufo-reports). We will be only looking at data for the US. You can get the zipped file here: [ufos.zip](ADD DATA LOCATION).

To get the data, you will need to upload it into your enviroment. The *upload* button will be under the *files* tab on the lower right. Find
# the zip file called `ufos` on your computer and upload it. You should see `ufos.csv` appear in your files.

Let's read in the csv as a [tibble](https://tibble.tidyverse.org/) and store it as a variable which we will use to call it for the rest of the lab. 

```{r import-data}
ufos <- as_tibble(read_csv("ufos.csv"))
```

You should see `ufos` show up as data set in your *Global Enviroment* on the upperright of your screen. If you click on it, you can see the data.

<p class = "note"> All of the functions we will be using are well documented. Either plug them into Google with the letter `r` or run them in the console with a `?`in front. (e.g. `as_tibble r` or `?as_tibble()`)</p>
...
Congratulations on finishing the tutorial! Now see if you can think up and answer so of your own questions! Or delve a little deeper into **R** with one of these books:
